...
namespace Simple_Code_Editor
{
    ...
    public class AdvancedRichTextBox : XRichTextBox
    {
        ...
        private char _skipQuotationCharacter;
        ...
        private YazanLib.Controls.XListView menu;
        int wordStart = 0;
        private YazanLib.Controls.DirectedConnection udc;
        ...
        public AdvancedRichTextBox()
        {
            ...
            StopColoring = false;
            Saved = true;
            HasSkipQuotationCharacter = false;
            ...
            menu = new YazanLib.Controls.XListView();
            menu.Visible = false;
            this.Controls.Add(menu);
            udc = new YazanLib.Controls.DirectedConnection(this, menu);
            System.Collections.Generic.List<YazanLib.Controls.Windows_Keys> CI = new System.Collections.Generic.List<YazanLib.Controls.Windows_Keys>();
            CI.Add(YazanLib.Controls.Windows_Keys.VK_DOWN);
            CI.Add(YazanLib.Controls.Windows_Keys.VK_UP);
            CI.Add(YazanLib.Controls.Windows_Keys.VK_RETURN);
            Messages.Add(YazanLib.Controls.Windows_Messages.WM_KEYDOWN);
            udc.Keys = CI;
            udc.Connected = false;
            menu.GotFocus+=new System.EventHandler(MenuGotFocus);
            menu.EnterPressed += new System.EventHandler(MenuEnterPressed);
            //((YazanLib.Controls.XListView)menu).Messages.Add(YazanLib.Controls.Windows_Messages.WM_KEYDOWN);
        }

        private void SetupContantMenu()
        {
            menu.Keywords = _keywords;
        }
        ...
        private void MenuEnterPressed(object sender,System.EventArgs e)
        {
            if (menu.SelectedItems.Count > 0)
            {
                ...
                if (wordStart == 0)
                {
                    ...
                    SelectionLength = start - wordStart;
                }
                else
                {
                    // if(Text[wordStart] == '\n')
                    SelectionStart = wordStart + 1;
                    SelectionLength = start - wordStart - 1;
                    // else

                }
                SelectedText = menu.SelectedItems[0].Text;
                //menu.Visible = false;
                udc.Connected = false;
                PaintControl = true;
            }
        ...
        {
            if (from == to - 1 || from > to - 1 || to - 1 >= Text.Length)
            {
                menu.Visible = false;
                udc.Connected = false;
                return;
            }
            menu.FillListWith(SubString(from, to - 1).Trim(),!_rules.HasFlag(System.Windows.Forms.RichTextBoxFinds.MatchCase));
            System.Drawing.Point cur = GetPositionFromCharIndex(SelectionStart - 1);
            menu.Location = new System.Drawing.Point(cur.X + Location.X, cur.Y + Location.Y + (int)(Font.Size * 2));
            if (menu.Items.Count > 0)
            {
                udc.Connected = true;
                menu.Items[0].EnsureVisible();
        ...
        public override void OnBackspacePressed(int selection)
        {
            //wordStart = SelectionStart;
            base.OnBackspacePressed(selection);
        }
        ...
        protected override void OnTextChanged(System.EventArgs e)
        {
            ...
            if (ShowMenu)
            {
                wordStart = GetFirstAttachableBeforeIndex(SelectionStart - 1);
        ...
        private void MenuGotFocus(object o, System.EventArgs e)
        {
            Focus();
        }
        ...
        private int GetFirstAttachableBeforeIndex(int index)
        {
            for (int i = index; i > 0; i--)
            {
                if (SearchPerformer.Attachable(Text[i]) == 1)
                    return i;
            }
            return 0;
        }
        ...
        private void ColoringText()
        {
            ...
            int start = ProcessingStart;//the start of line
            int end = ProcessingEnd;//end of the line
            PaintControl = false;//stop painting the control
            int find_at = start;
            for (int i = 0; i < _keywords.Count; i++)
            {
                ...
                while (find_at != -1)
                {
                    find_at = Find(_keywords[i].Text, find_at, end, _rules);//find the keyword
                    if (find_at > 0)
                        if (Text[find_at - 1] == '_')//if the keyword attached with '_' before it
                        {
                        ...
                        else
                        {
                            if (SelectionStart == Text.Length)
                                if (Text[find_at + (_keywords[i].Text.Length)] == '_')//if the keyword attached with '_' after it
                                {
                                    find_at += _keywords[i].Text.Length - 1;
                                    continue;
                                }
                        }
                    if (find_at != -1)//the keyword found
                    {
                        if (SelectionColor != _quotationColor && SelectionColor != _commentsColor)//the keyword not in comments or quotation
                        {
                        ...
                        find_at += _keywords[i].Text.Length - 1;//find next
                    }
                    else
                        break;
                }
        ...
        private void ProcessQuotationAndComments()
        {
            ...
            int length = 0;
            int currentSelection = SelectionStart;
            System.Drawing.Color currentColor = SelectionColor;//save current stat
            ...
            {
                if (Text[i] == _secondQuotationCharacter && !skip1)//if a quotation char and a no quotation opened
                {
                    if (i > 0 && HasSkipQuotationCharacter)
                        if (Text[i - 1] == _skipQuotationCharacter)//the char at i - 1 is a skip quotation char
                            continue;//skip
                    if (i != 0)//
                    {
                        if (Text[i - 1] != '\\')
                ...
                {
                    if (i > 0 && HasSkipQuotationCharacter)
                        if (Text[i - 1] == _skipQuotationCharacter)
                            continue;
                    if (i != 0)
                    {
                        if (Text[i - 1] != '\\')
        ...
        public char SkipQuotationCharacter
        {
            get { return _skipQuotationCharacter; }
            set { _skipQuotationCharacter = value; }
        }

        public bool HasSkipQuotationCharacter
        {
            get;
            set;
        }
